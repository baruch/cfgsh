...
static char const rcsid[] = "(C) 2004 Codefidence Ltd. $Id$";
...
COMMAND network_commands[] = {
  { "interface", com_int, "Enter interface configuration mode: interface [interface]", interface_completion_matches, NULL},
...
void set_prompt(char * prompt_add) {
  ...
  if(prompt_add)
    snprintf(prompt, PROMPT_SIZE, "%s %s%s", conf->hostname, prompt_add, PROMPT);
  else
    snprintf(prompt, PROMPT_SIZE, "%s%s", conf->hostname, PROMPT);
...
int find_ifs(void) {
  ...
  if(!( dev_file = fopen(PROC_NET_DEV, "r")))
    {
      printf("Failed to open proc/net/dev file %s\n", strerror(errno));
      exit(1);
    }
  ...
  while (fgets(buf, sizeof(buf), dev_file)) {
    ...
    if(!strncmp(name, IFNAMEBASE, 3)) i++;
  }
...
int write_resolver(char *ns1, char *ns2, char * search)
{
  ...
  if(strncmp(search, "none", 5))
    fprintf(file, "search %s\n", search);

  if(strncmp(ns1, "none", 5))
    fprintf(file, "nameserver %s\n", ns1);

  if(strncmp(ns2, "none", 5))
    fprintf(file, "nameserver %s\n", ns2);
...
usage(char * name) {
  ...
  printf("Copyright 2004 Codefidence Ltd. http://codefidence.con\n\n");
...
int main(int argc, char * argv[])
{
  ...
  if((argc == 2) && (!strncmp(argv[1], "setup", 6)) ) {

    setup = 1;
    show_prompt = 0;

    if(!( rl_instream = fopen(CONFIG_PATH, "r")))
      {
	printf("Failed to open config file %s\n", strerror(errno));
	exit(1);
      }
    
  } else if ((argc == 2) && (!strncmp(argv[1], "clean", 6)) ) {    
...
COMMAND * find_command (char *name)
{
  ...
  for (i = 0; commands[i].name; i++)
    if (strncmp (name, commands[i].name, strlen(commands[i].name) + 1) == 0)
      return (&commands[i]);
...
char ** cfgsh_completion (const char *text, int start, int end)
{
  ...
  if (start == 0)
    ...
    for (i = 0; commands[i].name; i++)
      {
	if (!*cmd || (strncmp (cmd, commands[i].name, strlen(commands[i].name) + 1) == 0))
	  {
...
int getifr(char * arg, char * ifrname, unsigned int * ifr)
{
  ...
  while (arg[i] && whitespace (arg[i])) i++;
...
int com_ip (char *arg)
{
  ...
  if (*arg) {
    ...
    if(!strncmp(conf->bcast[current_ifr], "auto", 5)) {
...
int com_show(char * arg)
{
  ...
  if (*arg) {
    ...
    if(!strncmp(arg, SHOW_OPT_INTERFACES, sizeof(SHOW_OPT_INTERFACES))) {
    ...
    if(!strncmp(arg, SHOW_OPT_ROUTES, sizeof(SHOW_OPT_ROUTES))) {
    ...
    if(!strncmp(arg, SHOW_OPT_RESOLVER, sizeof(SHOW_OPT_RESOLVER))) {
    ...
    if(!strncmp(arg, SHOW_OPT_CONFIG, sizeof(SHOW_OPT_CONFIG))) {
...
int com_dhcp (char *arg)
{
  ...
  if(strncmp(arg, DHCP_OPT_ON, sizeof(DHCP_OPT_ON)) == 0) {
  ...
  if(strncmp(arg, DHCP_OPT_IPONLY, sizeof(DHCP_OPT_IPONLY)) == 0) {
  ...
  if(strncmp(arg, DHCP_OPT_OFF, sizeof(DHCP_OPT_OFF)) == 0) {
...
int com_help (char *arg)
{
  ...
  for (i = 0; commands[i].name; i++)
    {
      if (!*arg || (strncmp (arg, commands[i].name, strlen(commands[i].name) + 1) == 0))
        {
...
int com_hostname (char *arg)
{
  ...
  snprintf(conf->hostname, size, arg); 
